...
export default function MovimientoModal({ open, onClose, item, id }) {
  ...
  const handleCantidadMovimiento = (e) => {
    const value = parseInt(e.target.value); // Ensure value is a number

    // Calculate proportional kilos based on new unidades and item data
    const newKilos = Math.round((item.kilos / item.unidades) * value);

    // Enforce maximum values based on item data
